...
class ReplayBufferLSTM3:
    ...
    def push(self, episode_length, hidden_in, hidden_out, state, action, last_action, reward, next_state, done):
        if len(self.buffer) < self.capacity:
            self.buffer.append(None)
        self.buffer[self.position] = (episode_length, hidden_in, hidden_out, state, action, last_action, reward, next_state, done)
        self.position = int((self.position + 1) % self.capacity)  # as a ring buffer

    def sample(self, batch_size):
        epi, s_lst, a_lst, la_lst, r_lst, ns_lst, hi_lst, ci_lst, ho_lst, co_lst, d_lst=[],[],[],[],[],[],[],[],[],[],[]
        batch = random.sample(self.buffer, batch_size)
        for sample in batch:
            episode_length, (h_in, c_in), (h_out, c_out), state, action, last_action, reward, next_state, done = sample
            epi.append(episode_length)
            s_lst.append(state)
            a_lst.append(action)
            la_lst.append(last_action)
            r_lst.append(reward)
        ...
        return epi, hidden_in, hidden_out, s_lst, a_lst, la_lst, r_lst, ns_lst, d_lst
